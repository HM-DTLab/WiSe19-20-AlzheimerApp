...
export class DataServiceService {
  ...
  putQrInfo(id: number, email: string, title: string, content: string, hasText: boolean) {
    ...
    // Request URL fuer Gateway-Api (Method PUT)
    const url = this.apiUrlPut.concat('?id=').concat(id.toString())
                              // .concat('&username=').concat(email)
                              .concat('&title=').concat(title)
                              .concat('&contentText=').concat(content)
                              .concat('&hasText=').concat(hasText.toString());
